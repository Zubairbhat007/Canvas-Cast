...
function CanvasCastApp() {
  ...
  const refreshDevices = useCallback(async () => {
    if (!navigator.mediaDevices?.enumerateDevices) {
      setMediaError('The current browser does not support media device enumeration.');
      return;
    }
    ...
    try {
      ...
      setMediaError('Unable to read the camera or microphone device list.');
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (!navigator.mediaDevices?.getUserMedia) {
      setMediaError('The current browser does not support camera access.');
      return;
    }
    ...
    navigator.mediaDevices
      ...
      .catch(() => {
        if (!disposed) {
          ...
          setMediaError('Failed to open camera. Please check permissions or device usage.');
        }
      });
  ...
  useEffect(() => {
    ...
    if (!navigator.mediaDevices?.getUserMedia) {
      setMediaError('The current browser does not support microphone access.');
      return;
    }
    ...
    navigator.mediaDevices
      ...
      .catch(() => {
        if (!disposed) {
          ...
          setMediaError('Failed to open microphone. Please check permissions or device usage.');
        }
      });
    ...
    <main className="desktop-only-notice" aria-labelledby="desktop-only-title">
      <section className="desktop-only-notice__card">
        ...
        <p className="desktop-only-notice__cn">
          CanvasCast is currently designed for desktop browsers. Please open it on a computer to get the full whiteboard drawing and recording experience.
        </p>
      </section>
    </main>
  );
